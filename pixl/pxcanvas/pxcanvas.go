...
func (pxCavas *PxCanvas) MouseToCanvasXY(ev *desktop.MouseEvent) (*int, *int) {
	bounds := pxCavas.Bounds()
	if !InBounds(ev.Position, bounds) {
		return nil, nil
	}

	pxSize := float32(pxCavas.PxSize)
	xOffset := pxCavas.CanvasOffset.X
	yOffset := pxCavas.CanvasOffset.Y
