...
func BuildNewMenu(app *AppInit) *fyne.MenuItem {
	return fyne.NewMenuItem("New", func() {
		sizeValidator := func(s string) error {
			...
			if err != nil {
				return errors.New("must be a positive intiger")
			}
			if width <= 0 {
		...
		widthEntery := widget.NewEntry()
		widthEntery.Validator = sizeValidator

		heightEntery := widget.NewEntry()
		heightEntery.Validator = sizeValidator

		widthFormEntery := widget.NewFormItem("width", widthEntery)
		heightFormEntery := widget.NewFormItem("height", heightEntery)

		formItems := []*widget.FormItem{widthFormEntery, heightFormEntery}

		dialog.ShowForm("New Image", "Create", "Cancel", formItems, func(ok bool) {
			if ok {
				pixelWidth := 0
				pixelHeight := 0
				if widthEntery.Validate() != nil {
					dialog.ShowError(errors.New("invalid width"), app.PixlWindow)
				} else {
					pixelWidth, _ = strconv.Atoi(widthEntery.Text)
				}
				if heightEntery.Validate() != nil {
					dialog.ShowError(errors.New("invalid height"), app.PixlWindow)
				} else {
					pixelWidth, _ = strconv.Atoi(heightEntery.Text)
				}
