...
func pbEnteryToMdbEntry(pbEntry *pb.EmailEntry) mdb.EmailEntry {
...
func (s *MailServer) GetEmailBatch(ctx context.Context, req *pb.GetEmailBatchRequest) (*pb.GetEmailBatchResponse, error) {
	...
	mdbEnteries, err := mdb.GetEmailBatch(s.db, params)
	if err != nil {
		return &pb.GetEmailBatchResponse{}, err
	}

	pbEnteries := make([]*pb.EmailEntry, 0, len(mdbEnteries))
	for i := 0; i < len(mdbEnteries); i++ {
		entry := mdbEntryToPbEntry(&mdbEnteries[i])
		pbEnteries = append(pbEnteries, &entry)
	}
	return &pb.GetEmailBatchResponse{EmailEnteries: pbEnteries}, nil
}
...
func (s *MailServer) UpdateEmail(ctx context.Context, req *pb.UpdateEmailRequest) (*pb.EmailResponse, error) {
	...
	entry := pbEnteryToMdbEntry(req.EmailEntry)
